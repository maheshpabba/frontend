...
export default function Chat() {
    ...
    useEffect(()=> {
        const ws=new WebSocket("ws://dcaf.cisco.com:5000/ws/ChatCompletion/"+clientId)
        ws.onmessage = (e) => {
            console.log(e.data)
    ...
    return (
      <>
          <div className="h-screen grid grid-cols-3 pl-2">
                  <div className="col-span-2 flex flex-col w-full">
                      <div className="flex-grow overflow-y-scroll md:overflow-scroll bg-gray-100">
                      {messages.map((mess, index) => (
                        <div className={`message ${mess.user ? 'chat chat-start':'chat chat-end'}`} key={index}>
                            <div className="chat-bubble">
                                ...
                                {
                                !mess.user && mess.text =='' ?
                                <span className="loading loading-dots loading-lg"></span> : mess.text
                                }  
                                </pre>
                            </div>
                        </div>
