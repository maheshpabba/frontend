...
function UserMenu({ session }) {
	...
	return (
		<a
			href={`/login?path=${location}`}
			className="font-light btn btn-ghost text-primary"
		>
			Login
		</a>
	)
...
export default function Navbar() {
	...
	const session = useSWR("/api/session/mine", () =>
		fetch("/api/sessions/mine").then((res) => res.json()),
	)

	return (
		<div className="fixed left-0 z-20 w-full h-16 border-b navbar border-b-base-200 bg-base-100">
